...
`WhisperEngine` (`app/transcription_engine.py`) wraps `whisper.transcribe()`. It converts Whisper's segments into `Segment(id, start, end, text)`, with times in seconds rounded to 10 ms and empty segments dropped. The model loads lazily, on first use rather than at import, so the API starts instantly and a mock-engine deployment never installs PyTorch.
...
**The merge (`merge_chunk_results`).** Speech in an overlap is transcribed twice, once by each chunk, so the merge has to keep exactly one copy:

1. **Shift** every segment by its chunk's start time, putting it on the file's timeline.
2. **Cut each overlap at its midpoint**, rather than where the earlier chunk ends. Audio right at a chunk's edge has the least context, so that is where words get clipped or misheard. With the cut at the midpoint, every kept segment is at least `overlap/2` seconds from the edge of the chunk it came from.
3. **Keep each segment by its centre point.** A segment is kept only if:
   - its centre is before this chunk's cut, and
   - its centre is at or after the end of the transcript assembled so far.
4. **Clamp** a kept segment's start to the previous segment's end, so timestamps never overlap or go backwards.

*What if a word falls exactly on the boundary?*
- **A segment centred exactly on the cut** belongs to the later chunk, because each chunk keeps only segments centred *before* its cut. The segment is kept once, never twice and never dropped.
- **A phrase that crosses the cut** is kept once, by the chunk that holds most of it.
- **A word clipped at a chunk's edge** (e.g. `"extraord"`) is centred past the cut, so it's dropped in favour of the next chunk's complete `"extraordinary"`.

Each of these cases has its own test in `tests/test_transcription_engine.py`.

When both chunks divide the seam into the same segments (the normal case: same speech, same pauses), the merged output is **identical** to transcribing the file in one pass. `test_merge_chunk_results_dedupes_overlap_and_spans_full_duration` checks this on 25 s, 123 s and 600 s files. For the known limit when the two chunks divide the seam differently, see [Known limitations](#known-limitations).
